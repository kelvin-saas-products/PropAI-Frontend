          ...
          {user ? (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 hover:bg-bg px-3 py-1.5 rounded-xl transition-colors"
              >
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl border border-subtle shadow-float py-1.5 animate-fadeIn">
                  <div className="px-4 py-2.5 border-b border-subtle">
                    ...
                    <span className="inline-block mt-1 text-[10px] font-semibold bg-bg text-muted px-2 py-0.5 rounded-full uppercase tracking-wider">
                      {user.role}
                    </span>
                  </div>
                  {user.role === 'agent'
                    ? <Link href="/dashboard/agent" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-muted hover:text-ink hover:bg-bg transition-colors">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                        Agent dashboard
                      </Link>
                    : <Link href="/dashboard" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-muted hover:text-ink hover:bg-bg transition-colors">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                        My dashboard
                      </Link>
                  }
                  <Link href="/dashboard/saved" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-muted hover:text-ink hover:bg-bg transition-colors">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                    Saved properties
                  </Link>
                  <Link href="/account/settings" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-muted hover:text-ink hover:bg-bg transition-colors">
            ...
            <>
              <Link href="/auth/sign-in" className="text-sm font-medium text-ink hover:text-muted transition-colors">Sign in</Link>
...
<<<<<<< HEAD
          ...
          <div className="border-t border-subtle pt-3 space-y-2">
            {user ? (
              ...
              <>
                <Link href="/auth/sign-in" className="block text-sm font-medium text-ink">Sign in</Link>
                <Link href="/auth/register" className="block bg-ink text-white text-sm font-semibold px-4 py-2.5 rounded-lg text-center">Join free</Link>
              </>
            )}
          </div>
=======
