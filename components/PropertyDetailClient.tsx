...
import type { Property, BadgeColor } from '@/lib/types'
    ...
    <div>
      ...
      <div className="bg-ink rounded-2xl p-5 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { icon: '🌴', title: 'Outdoor Verandah w/ Palm Garden', body: 'Deep rear verandah with established Bangalow palms — perfect for year-round entertaining.' },
            { icon: '☀️', title: 'North-Facing Rear Aspect', body: 'Satellite confirms north-northwest orientation, flooding the living area with afternoon light.' },
            { icon: '⚡', title: 'Solar Panels Detected (6.6kW)', body: '18 panels on the north roof plane — est. $800–$1,200/year in feed-in tariffs.' },
            { icon: '🏊', title: 'Pool Preparation Slab', body: 'Concrete slab with plumbing stubouts in rear garden suggests a planned pool installation.' },
          ].map(obs => (
            <div key={obs.title} className="bg-white/08 rounded-xl p-3.5 flex gap-3">
...
const priceHistory = [
  { year: '2013', price: 675 }, { year: '2015', price: 820 },
  { year: '2019', price: 1000 }, { year: '2022', price: 1150 }, { year: '2025', price: 1250 },
]
...
function ShopsTab() {
  return (
    <div>
      <Section title="Cafés & Dining">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { icon: '☕', name: 'The Boathouse Mosman', type: 'Café · 0.5km', rating: '★ 4.8 · 1,100+ reviews' },
            { icon: '🍣', name: 'Aqua Dining', type: 'Japanese · 1.1km', rating: '★ 4.7 · 820+ reviews' },
            { icon: '🥂', name: 'Bathers Pavilion', type: 'Fine Dining · 0.8km', rating: '★ 4.9 · 2,200+ reviews' },
            { icon: '🥐', name: 'Mosman Patisserie', type: 'Bakery · 0.3km', rating: '★ 4.6 · 540+ reviews' },
          ].map(s => (
      ...
      <Section title="Shopping & Retail">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { icon: '🏬', name: 'Military Road Strip', desc: '150+ shops · 0.4km · Fashion, food, services' },
            { icon: '🛒', name: 'Woolworths Mosman', desc: 'Supermarket · 0.5km · Open 7am–10pm' },
            { icon: '🛍', name: 'Spit Junction', desc: 'Retail precinct · 2.1km · Major anchors' },
            { icon: '🌿', name: 'Mosman Farmers Market', desc: 'Monthly · 0.3km · Sundays 8am–1pm' },
          ].map(s => (
            <Card key={s.name} className="flex items-center gap-3 !p-4">
              <span className="text-2xl">{s.icon}</span>
              <div>
...
function HealthTab() {
  return (
    <div>
      <Section title="Hospitals & Medical">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
...
const growthData = [
  { year: '2015', v: 35, f: false }, { year: '2016', v: 52, f: false }, { year: '2017', v: 44, f: false },
  { year: '2018', v: 28, f: false }, { year: '2019', v: 18, f: false }, { year: '2020', v: 22, f: false },
  { year: '2021', v: 65, f: false }, { year: '2022', v: 55, f: false }, { year: '2023', v: 60, f: false },
  { year: '2024', v: 72, f: false }, { year: '2025F', v: 65, f: true }, { year: '2026F', v: 70, f: true },
]
            ...
            {[
              ...
              <div key={f.label}>
                ...
                <input type="number" value={f.value} step={f.step}
                  ...
                  className="w-full px-3.5 py-2.5 border border-subtle rounded-xl text-sm text-ink bg-bg outline-none focus:border-ink/30 transition-colors"/>
              </div>
            ))}
            <div>
              ...
              <select value={term} onChange={e => setTerm(parseInt(e.target.value))}
                className="w-full px-3.5 py-2.5 border border-subtle rounded-xl text-sm bg-bg text-ink outline-none">
...
export default function PropertyDetailClient({ property }: { property: Property }) {
  ...
  return (
    <div className="min-h-screen bg-bg">
      {/* Top bar */}
      <div className="bg-white border-b border-subtle px-5 lg:px-8 h-12 flex items-center justify-between">
      ...
      {/* GALLERY — matching screenshot 4 exactly */}
      <div className="grid grid-cols-2 lg:grid-cols-[2fr_1fr_1fr] grid-rows-2 gap-1" style={{ height: 400 }}>
        <div className="row-span-2 relative overflow-hidden">
          <Image src={property.images[0]} alt="Main" fill className="object-cover" sizes="50vw" priority/>
          {/* Overlays on main image */}
          <div className="absolute bottom-4 left-4 flex gap-2">
            <span className="bg-white/90 backdrop-blur text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-green inline-block"/>
              {property.aiMatch}% AI Match
            </span>
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${BADGE_STYLES[property.badge.color]}`}>
        ...
        {property.images.slice(1,5).map((img, i) => (
          <div key={i} className="relative overflow-hidden group">
            <Image src={img} alt={`Photo ${i+2}`} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="25vw"/>
            {i === 3 && (
              <div className="absolute inset-0 bg-ink/50 flex items-center justify-center cursor-pointer">
                <span className="text-white text-sm font-semibold">View All 7 Photos</span>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8">
        ...
        <div>
          {/* Property header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-ink mb-1">{property.title}</h1>
        ...
        <div className="space-y-4 lg:sticky lg:top-4 lg:self-start">
          ...
          <div className="bg-ink rounded-2xl p-5 text-white">
            <p className="text-xs text-white/50 mb-1">Asking Price</p>
            <p className="text-3xl font-black mb-5">{property.price}</p>
          ...
          <Card>
            ...
            <div className="space-y-2">
              <button className="w-full flex items-center gap-3 py-2.5 px-3 rounded-xl border border-subtle hover:bg-bg transition-colors text-sm text-ink">
              ...
              <button className="w-full flex items-center gap-3 py-2.5 px-3 rounded-xl border border-subtle hover:bg-bg transition-colors text-sm text-ink">
                <svg className="w-4 h-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                Email Agent
              </button>
              <button className="w-full flex items-center gap-3 py-2.5 px-3 rounded-xl border border-subtle hover:bg-bg transition-colors text-sm text-ink">
                <svg className="w-4 h-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                Request Callback
              </button>
            </div>
          </Card>
          ...
          <Card className="!bg-bg border-0">
            ...
            <p className="font-bold text-ink">{property.openHome}</p>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-2.5">
              ...
              <div className="pt-1 border-t border-subtle flex justify-between items-center">
                <span className="text-xs text-muted">Capital Growth</span>
                <span className="text-sm font-black text-green">{property.scores.growth}</span>
              </div>
            </div>
          </Card>
        </div>
