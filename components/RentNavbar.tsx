...
// ── Rent-specific price ranges (weekly rent) ──────────────────────
const PRICE_RANGES = [
  { label: 'Any price',          min: '', max: ''       },
  { label: 'Under $500/wk',      min: '', max: '500'    },
  { label: '$500 – $800/wk',     min: '500',  max: '800'    },
  { label: '$800 – $1,200/wk',   min: '800',  max: '1200'   },
  { label: '$1,200 – $2,000/wk', min: '1200', max: '2000'   },
  { label: '$2,000 – $3,500/wk', min: '2000', max: '3500'   },
  { label: 'Over $3,500/wk',     min: '3500', max: ''       },
]

const BED_OPTIONS = ['Any', '1+', '2+', '3+', '4+', '5+']

const PROPERTY_TYPES = ['House', 'Apartment', 'Townhouse', 'Villa', 'Condo', 'Penthouse', 'Studio', 'Unit']
    ...
    <header className="sticky top-0 z-50 bg-white border-b border-subtle">
      <div className="max-w-[1400px] mx-auto px-5 lg:px-8 h-14 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 flex-1 justify-center flex-wrap">
          ...
          {/* Rent dropdown – always active/selected */}
          <FilterDropdown label="Rent" active width="w-48">
            ...
            {['Buy', 'Rent'].map(opt => (
              <Link
                ...
                className={`flex items-center gap-2 w-full px-3 py-2 rounded-xl text-sm font-semibold transition-colors ${
                  opt === 'Rent' ? 'bg-ink text-white' : 'text-muted hover:text-ink hover:bg-surface'
                }`}
              >
                {opt === 'Buy' ? '🏷' : '🔑'} {opt}
              </Link>
          ...
          {/* Price (weekly rent) */}
          <FilterDropdown label={priceLabel} active={priceActive} width="w-60">
            <p className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Weekly rent</p>
